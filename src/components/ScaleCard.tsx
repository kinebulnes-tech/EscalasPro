...
export default function ScaleCard({ scale, onClick }: ScaleCardProps) {
  return (
    <div
      onClick={onClick}
      className="bg-white rounded-xl shadow-md border border-gray-100 p-6 hover:shadow-xl hover:-translate-y-1 hover:border-teal-400 transition-all duration-300 cursor-pointer group"
    >
      <div className="flex items-start gap-4">
        {/* Ícono con fondo que cambia al pasar el mouse */}
        <div className="bg-teal-50 p-3 rounded-xl group-hover:bg-teal-100 transition-colors duration-300">
          <FileText className="w-6 h-6 text-teal-600" />
        </div>
        
        <div className="flex-1">
          {/* Título que se ilumina en turquesa */}
